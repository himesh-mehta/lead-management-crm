  ...
  return (
    <>
      ...
      <aside
        ...
        className={`fixed top-16 bottom-0 left-0 z-50 md:z-30 flex flex-col glass-sidebar transition-all duration-300 ease-in-out ${
              ...
              <div key={sec.title} className="space-y-1">
                ...
                {sec.items.map(({ name, path, icon: Icon, end }) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center ${isOpen ? 'justify-between px-3.5' : 'justify-center px-1'} py-2 rounded-xl text-sm font-semibold tracking-wide transition-all duration-200 border-l-2 group ${
                        isActive
        ...
        <div className="border-t border-indigo-500/20 dark:border-indigo-950/50 bg-gray-50/30 dark:bg-slate-900/10 flex flex-col gap-2 p-3 flex-shrink-0">
          {isOpen ? (
            <div className="flex items-center gap-3 p-1">
              <div className="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 font-extrabold flex items-center justify-center border border-indigo-200/30 text-sm flex-shrink-0">
                HM
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 font-semibold truncate uppercase mt-0.5">
                  Super Admin
                </p>
              </div>
