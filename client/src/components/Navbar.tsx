...
import { Menu, Plus, Search, Bell, Clock, LogOut, Settings, Shield } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 h-16 w-full glass-panel border-b border-gray-200 dark:border-slate-805 flex items-center justify-between px-6 transition-colors duration-300">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Title */}
        <h2 className="hidden sm:block text-base font-bold text-gray-955 dark:text-gray-100 tracking-tight">
          {title}
        </h2>
      </div>
      ...
      <div className="flex items-center gap-4 flex-1 justify-end">
        ...
        {onAddLeadClick && (
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold text-white bg-indigo-650 hover:bg-indigo-700 shadow-sm active:scale-[0.98] transition-all"
            title="Create Lead"
          >
            <Plus size={14} />
        ...
        <form onSubmit={handleSearchSubmit} className="relative max-w-xs w-full hidden md:block mr-2">
          ...
          <input
            ...
            className={`w-full pl-9 pr-12 py-1.5 rounded-xl text-xs bg-gray-50 dark:bg-slate-900/50 border transition-all duration-200 text-gray-800 dark:text-slate-200 focus:outline-none ${
              searchFocused 
                ...
                : 'border-gray-200 dark:border-slate-850'
            }`}
          />
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-slate-800 text-gray-550 dark:text-slate-400 font-mono pointer-events-none">
            ⌘K
          </span>
        </form>
              ...
              <div className="divide-y divide-gray-50 dark:divide-slate-850 max-h-60 overflow-y-auto">
                {mockNotifications.map(notif => (
                  <div key={notif.id} className="p-3 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors flex items-start gap-2.5">
                    <span className={`w-1.5 h-1.5 rounded-full mt-1.5 ${notif.unread ? 'bg-indigo-650 dark:bg-indigo-400' : 'bg-transparent'}`} />
                    <div className="flex-1">
                      <p className="text-xs text-gray-800 dark:text-slate-200 font-medium leading-relaxed">{notif.text}</p>
                      <span className="text-[10px] text-gray-400 dark:text-slate-550 flex items-center gap-1 mt-1">
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="w-8 h-8 rounded-full bg-indigo-650 text-white font-bold flex items-center justify-center text-xs shadow-sm ring-2 ring-indigo-600/10 hover:ring-indigo-600/30 transition-all">
              HM
            </div>
          </button>
