...
const KPICard: React.FC<KPIValProps> = ({ title, value, growth, icon: Icon, color, sparklineData }) => {
  ...
  const iconColorMap = {
    indigo: 'text-indigo-650 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-950/20 border-indigo-100 dark:border-indigo-900/30',
  ...
  const sparkStroke = color === 'indigo' ? 'rgb(99, 102, 241)' : color === 'green' ? 'rgb(16, 185, 129)' : color === 'red' ? 'rgb(239, 68, 68)' : 'rgb(59, 130, 246)';
...
const Stats: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(16);
      pdf.setTextColor(99, 102, 241); // Indigo theme color
    ...
    <div className="space-y-6 p-4 sm:p-6 max-w-[1400px] mx-auto animate-fade-in pb-24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-150 dark:border-slate-800 pb-5">
        ...
        <motion.button
          ...
          disabled={exporting}
          className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-xl transition-colors shadow-xs self-start sm:self-center"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-soft">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.monthlyGrowth} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                <defs>
                ...
                <Tooltip
                  ...
                  formatter={(value: any, _name: any, props: any) => [
                    `${value} leads (${props.payload.growthMoM}% MoM)`,
                    'New Leads'
                  ]}
                />
                <Area
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-soft">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.conversionTrend} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(156, 163, 175, 0.1)" />
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  {stats.sourceDistribution.map((_entry: any, index: number) => (
                    <Cell
                      key={`cell-${index}`}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.revenueForecast} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="Qualified" fill="rgb(139, 92, 246)" stackId="a" radius={[0, 0, 0, 0]} />
                <Bar dataKey="Converted" fill="rgb(16, 185, 129)" stackId="a" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-gray-150 dark:border-slate-800 text-gray-400 font-bold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="py-2.5 text-center text-indigo-500">Qualified</th>
                ...
                {stats.topCompanies.length === 0 ? (
                  ...
                  stats.topCompanies.map((c: any, index: number) => (
                    <tr key={index} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/20">
                      ...
                      <td className="py-3 text-center text-indigo-650 dark:text-indigo-400 font-bold">{c.qualified}</td>
